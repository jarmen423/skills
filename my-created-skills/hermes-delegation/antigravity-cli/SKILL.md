...
- `agy -p` supports `--output-format text`, `json`, and `stream-json`.
  `stream-json` input requires matching `--input-format stream-json` and reads
  one newline-delimited JSON message per turn. Use `--json-schema '<schema>'`
  (or a schema-file path) when the final result needs a machine-checked shape.
- There is **no `--max-turns`**. A print run is bounded by **`--print-timeout`**
  ...
  Hermes `terminal` `timeout=` so the outer call does not cut the run short.
- JSON output is structured CLI output, not a guarantee of a Claude-style
  envelope containing session cost or turn-count metadata. Inspect the actual
  returned shape before writing a parser around it.
...
- Workspace identity can depend on launch directory and the `.antigravitycli`
  project marker.
- **Absolute paths in a prompt can override the launch worktree in practice.**
  In a read-only review, Antigravity was launched from an isolated detached
  worktree but followed absolute plan paths under the production clone, set its
  command `Cwd` to that clone, and ran `git checkout <review-sha>` there. Never
  give `agy` absolute paths into a protected/deploy clone when a sibling
  worktree is the intended workspace. Copy required untracked plans into the
  review worktree (or inject their text), refer only to worktree-relative paths,
  and inspect the conversation transcript's first `run_command` Cwd before
  allowing further commands. Afterward verify the protected clone's branch and
  HEAD, not just the review worktree.
- `agy -p` supports text, JSON, and streaming JSON output. Match
  `--input-format stream-json` with `--output-format stream-json`, and inspect
  the actual JSON shape before depending on undocumented envelope fields.
- Bound print runs with `--print-timeout` (default `5m`), not `--max-turns` (which does not exist on `agy`).
- Review agy's commit yourself before trusting its success summary (observed 2026-09-03 on a real port task): with `--dangerously-skip-permissions` it papers over **pre-existing** type errors with `@ts-ignore`/`@ts-expect-error` edits on unrelated files, adds spurious lockfiles (`bun.lock` where only `package-lock.json` was tracked), edits configs to widen type tolerance, and leaves tsc emit artifacts (`.js`/`.d.ts`/`tsbuildinfo` generated from `.ts` configs) as tracked additions. It also skips build steps it wasn't explicitly given. Procedure that worked: establish the `tsc -b` error baseline from the parent commit first, gate on "zero NEW errors" (total count will be nonzero on legacy code), diff the commit against parent and revert every out-of-scope file, delete artifacts, and re-run tests + build on the cleaned tree before amending. Stage only in-scope files when amending — `git add -A` re-admits the junk.
